...
export default function Login() {
  ...
  async function handleSubmit(e: SyntheticEvent) {
    ...
    let data;
    try {
      const res = await fetch(`${BASE_API_ENDPOINT}/auth/log-in`, {
        method: 'POST',
        credentials: 'include',
        headers: {
    ...
    setErrors(errors);
    const { first, last, accessToken, roles } = data;
    login(first, last, accessToken, email, roles);

    const churchId = location.state?.from;

    if (churchId) {
      navigate(`/church/${churchId}`, { state: { from: 'login' } });
      return;
    }
